...
const MODES: { id: DemoMode; label: string; desc: string }[] = [
  { id: "support",    label: "Customer Support",  desc: "Triage · classify · respond" },
  { id: "listing",   label: "Real Estate Listing", desc: "Big 4 · CMA · doc scan" },
  { id: "investment", label: "RE Investment",      desc: "Deal scan · memo · ROI gate" },
];
...
const SUPPORT_TICKETS: TicketItem[] = [
  ...
  {
    id: "#TKT-4822",
    from: "marcus.james@brightwave.io",
...
const LISTING_TICKETS: TicketItem[] = [
  {
    id: "#LST-0091",
    from: "agent@cornerstone-realty.com",
    ...
    priority: "P1",
    _scenario: "listing_austin",
    _mode: "listing",
    label: "Residential · Austin",
    time: "just now",
    address: "4204 Manchaca Rd, Austin TX 78704",
    specs: "3BR · 2BA · 1,556 sqft · $485k",
  },
  {
    id: "#LST-0092",
    from: "agent@sfpacifichomes.com",
    ...
    priority: "P2",
    _scenario: "listing_sf",
    _mode: "listing",
    label: "Luxury · San Francisco",
    time: "8m ago",
    address: "1427 Sanchez St #6, San Francisco CA 94131",
    specs: "4BR · 3.5BA · 2,840 sqft · $2.1M",
  },
];
...
const INVESTMENT_TICKETS: TicketItem[] = [
  {
    id: "#INV-0047",
    from: "acquisitions@vestro.capital",
    ...
    priority: "P1",
    _scenario: "invest_flip",
    _mode: "investment",
    label: "Fix & Flip · Phoenix",
    time: "just now",
    address: "4412 W. Camelback Rd, Phoenix AZ 85031",
    specs: "3BR · 1BA · 1,580 sqft · Ask $215k",
  },
  {
    id: "#INV-0048",
    from: "acquisitions@vestro.capital",
    ...
    priority: "P2",
    _scenario: "invest_brrr",
    _mode: "investment",
    label: "BRRRR · Cleveland",
    time: "22m ago",
    address: "2281 E. 93rd St, Cleveland OH 44106",
    specs: "3BR · 2BA · 1,320 sqft · Ask $118k",
  },
];
...
// ── Pipeline node definitions per mode ─────────────────────────────────────────
...
function summarizeOutput(stepName: string, output: Record<string, unknown>): string {
  switch (stepName) {
    ...
    case "classify":  return `cat=${output.category} · intent=${output.intent} · conf=${output.confidence}`;
...
export default function LiveDemo() {
  ...
  const startEngine = useCallback((ticketIdx: number) => {
    ...
    // Reset all accumulated state synchronously via refs
  ...
  // Start only when section enters the viewport — users won't miss it mid-scroll
  useEffect(() => {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="rounded-lg border border-[rgba(255,255,255,0.07)] overflow-hidden bg-[#0e0e11]">
          ...
          <div className="flex items-center gap-2 px-4 h-9 border-b border-[rgba(255,255,255,0.07)] bg-[#111114]">
            ...
            <span className="text-[11px] text-[#52525b] ml-2" style={{ fontFamily: "var(--font-mono)" }}>
              fein-ai · {demoMode === "support" ? "customer-support-pipeline" : demoMode === "listing" ? "listing-content-pipeline" : "investment-analysis-pipeline"}
            </span>
          </div>
          ...
          <div className="flex items-center border-b border-[rgba(255,255,255,0.07)] bg-[#0c0c0f]">
            {MODES.map((m) => (
              <button
                ...
                className={`px-4 py-2.5 text-[10px] transition-all duration-150 border-b-2 flex-1 text-center ${
                  demoMode === m.id
              ...
              >
                ...
                <div className={`text-[8px] mt-0.5 ${demoMode === m.id ? "text-[#71917a]" : "text-[#3f3f46]"}`}>{m.desc}</div>
              </button>
            ))}
          </div>
          ...
          <div className="border-b border-[rgba(255,255,255,0.07)] bg-[#0a0a0d]">
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b border-[rgba(255,255,255,0.04)]">
              <div className="flex items-center gap-2 text-[10px] text-[#3f3f46]" style={{ fontFamily: "var(--font-mono)" }}>
                <span>{runId ? `${runId.slice(0, 18)}…` : "—"}</span>
                <span className="text-[#27272a]">·</span>
                <span className="text-[#27272a]">{ticket.id} · {ticket.label}</span>
              </div>
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] text-[#3f3f46] tabular-nums" style={{ fontFamily: "var(--font-mono)" }}>
                  {elapsed}
                </span>
            ...
            <div className="flex items-center gap-0 px-4 py-1.5 overflow-x-auto min-h-[28px]">
              {activeSteps.length === 0 ? (
                ...
                activeSteps.flatMap((step, idx) => {
                  const chip = (
                    <div key={step.stepIndex} className="flex items-center gap-1 flex-shrink-0">
                  ...
                  const sep = idx < activeSteps.length - 1
                    ? <span key={`sep-${step.stepIndex}`} className="text-[#27272a] text-[9px] mx-2 flex-shrink-0" style={{ fontFamily: "var(--font-mono)" }}>──</span>
                    : null;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_180px_1fr] gap-0 min-h-[420px]">
            ...
            {/* Left: queue + structured inputs */}
            <div className="p-5 border-b lg:border-b-0 lg:border-r border-[rgba(255,255,255,0.07)] flex flex-col gap-3">
              ...
              <div className="rounded border border-[rgba(255,255,255,0.05)] bg-[#0a0a0d] px-3 py-2.5">
                <p className="text-[11px] text-[#52525b] leading-relaxed" style={{ fontFamily: "var(--font-display)" }}>
                  {ticket.body}
                </p>
              </div>
  ...
  return (
    <div>
      ...
      <div className="rounded border border-[rgba(255,255,255,0.05)] bg-[#0a0a0d] divide-y divide-[rgba(255,255,255,0.04)]">
        {rows.map((row) => (
          <div key={row.key} className="flex items-center justify-between px-2.5 py-1">
            <span className="text-[9px] text-[#3f3f46]" style={{ fontFamily: "var(--font-mono)" }}>{row.key}</span>
            <span className="text-[9px] text-[#71717a] tabular-nums" style={{ fontFamily: "var(--font-mono)" }}>{row.value}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex gap-2">
          <button onClick={() => onEditAndApprove(editedDraft)} disabled={!editedDraft.trim()} className="flex-1 py-2 rounded bg-[#a3e635] text-[#0c0c0e] text-[11px] font-semibold hover:bg-[#bef264] disabled:opacity-40 disabled:cursor-not-allowed transition-colors duration-150" style={{ fontFamily: "var(--font-mono)" }}>
            Submit Edit →
          </button>
          <button onClick={() => setEditing(false)} className="px-3 py-2 rounded border border-[rgba(255,255,255,0.07)] text-[#52525b] text-[11px] hover:text-[#71717a] transition-colors duration-150" style={{ fontFamily: "var(--font-mono)" }}>
            Cancel
          </button>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-col gap-2">
        <button onClick={onApprove} className="w-full py-2 rounded bg-[#a3e635] text-[#0c0c0e] text-[11px] font-semibold hover:bg-[#bef264] transition-colors duration-150" style={{ fontFamily: "var(--font-mono)" }}>
          {mode === "support" ? "Approve →" : mode === "listing" ? "Publish →" : "Submit Offer →"}
        </button>
        <button onClick={() => setEditing(true)} className="w-full py-2 rounded border border-[rgba(255,255,255,0.1)] text-[#71717a] text-[11px] font-medium hover:border-[rgba(163,230,53,0.3)] hover:text-[#a3e635] transition-colors duration-150" style={{ fontFamily: "var(--font-mono)" }}>Edit & Approve</button>
        <button onClick={onReject} className="w-full py-2 rounded border border-red-900/30 text-red-400 text-[11px] font-medium hover:bg-red-950/20 transition-colors duration-150" style={{ fontFamily: "var(--font-mono)" }}>
...
function FinalResult({ draft, confidence, edited, mode, onReplay }: {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {draft && (
        <div className="flex-1 rounded border border-[rgba(255,255,255,0.07)] bg-[#111114] p-3 overflow-y-auto mb-3">
      ...
      <button onClick={onReplay} className="w-full py-2 rounded border border-[rgba(255,255,255,0.07)] text-[#52525b] text-[11px] hover:text-[#71717a] hover:border-[rgba(255,255,255,0.12)] transition-colors duration-150" style={{ fontFamily: "var(--font-mono)" }}>
        ↺ Run again
      </button>
    </div>
  );
...
function RejectedState({ onReplay }: { onReplay: () => void }) {
  return (
    <div className="flex flex-col h-full items-center justify-center gap-4">
      ...
      <button onClick={onReplay} className="mt-2 px-4 py-2 rounded border border-[rgba(255,255,255,0.07)] text-[#52525b] text-[11px] hover:text-[#71717a] hover:border-[rgba(255,255,255,0.12)] transition-colors duration-150" style={{ fontFamily: "var(--font-mono)" }}>
        ↺ Run again
      </button>
    </div>
  );
